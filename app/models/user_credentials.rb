...
class UserCredentials < ActiveRecord::Base
  ...
  encrypt_with_public_key :password, key_pair: Houston::Credentials.config.keypair
  ...
  def self.for(service)
    ...
    [credentials.login, credentials.password.decrypt(Houston::Credentials.config.passphrase)]
